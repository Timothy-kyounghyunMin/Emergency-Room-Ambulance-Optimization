...
#np.random.RandomState(seed_num-1)
...
class Hospital():
  ...
  def sample_treat_time(self):
    ...
    treat_time = self.random_state.exponential(self.mean_treat_time, 1)
    return treat_time
...
def updater(model_type: str):  
  ...
  for id in patient_dict.keys():
    ...
    if tau - patient_dict[id].created_time > patient_dict[id].destiny:
      etaw_toll += patient_dict[id].destiny #$#$#$#$#$#$#$# 수정 #$#$#$#$#$#$#$#
      etaw_toll += compute_etaw(patient_dict[id], patient_dict[id].decision, tau)[1] #### 죽은 환자의 예상 etaw도 toll에 포함한다면 ###   
  ...
  for key in hospitals_dict.keys():
    ...
    for i, patient in enumerate(hospitals_dict[key].queue[1:]):
      ...
      if tau - patient.created_time > patient.destiny:
        etaw_toll += patient.destiny #$#$#$#$#$#$#$# 수정 #$#$#$#$#$#$#$#
        
        expected_wait = ((i+1) + np.sqrt(i+1)) * hospitals_dict[key].mean_treat_time   #### 대기하다가 죽는 환자의 etaw를 toll에 포함한다면 ###
        etaw_toll += expected_wait   #### 대기하다가 죽는 환자의 etaw를 toll에 포함한다면 ###
...
end_time = 10*3600*1 # Simulation end time
...
while time_point < end_time:
    ...
    # np.random.seed(seed_num + time_point) ###################### 수정했음 (seed_num + time_point)
    destiny_temp = np.random.beta(alpha_skewed, beta_skewed, 1)[0]
